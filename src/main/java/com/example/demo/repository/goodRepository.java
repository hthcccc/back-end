...
public interface goodRepository  extends JpaRepository<Good,String>, JpaSpecificationExecutor<Good> {
    ...
    @Query(value = "select * from good where good_id in (select good_id from(select count(order_id) ct,good.good_id from good left outer join trade_order on good.good_id=trade_order.good_id where good.good_state='上架中' group by(good.good_id) order by ct desc limit 10) as t(ct,good_id))"
            ,nativeQuery = true)
    List<Good> getTop10();
