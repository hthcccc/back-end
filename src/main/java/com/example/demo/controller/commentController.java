...
//    }
    ...
    public Result newComment(@RequestParam("user_id") String user_id,
                           @RequestParam("good_id") String good_id,
                           @RequestParam("text") String text){
        return tmp.newComment(user_id,good_id,text);
    }

    @PostMapping("/replyComment")
    @ApiGroup(group = {"comment"})
    @ApiOperation(value = "回复评论",notes = "评论id，用户id，文字信息")
    public Result replyComment(@RequestParam("comment_id") String comment_id,
                           @RequestParam("user_id") String user_id,
                           @RequestParam("text") String text){
        return tmp.addComment(comment_id,user_id,text);
    }

    @PostMapping("/deleteComment")
    @ApiGroup(group = {"comment"})
    @ApiOperation(value = "删除评论",notes = "评论id，层数")
    public Result deleteComment(@RequestParam("comment_id") String comment_id,
                             @RequestParam("level") Integer level){
        return tmp.deleteComment(comment_id,level);
    }

    @GetMapping("/getAllByGood/{good_id}")
    @ApiGroup(group = {"good","comment"})
    @ApiOperation(value = "获取商品的所有评论（第一层）",notes = "商品id")
    public Result getAllByGood(@PathVariable String good_id){
    ...
    public Result listAllBelow(@PathVariable String comment_id){
